...
import { useRestartStrategy, useStrategyPerformance } from "@/api/strategy";
          ...
          {isStopped ? (
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                variant="ghost"
                size="sm"
                className="rounded-md px-2.5 py-1 font-medium text-foreground text-sm"
                onClick={(event) => {
...
const TradeStrategyGroup: FC<TradeStrategyGroupProps> = ({
  ...
  const { mutate: restartStrategy } = useRestartStrategy();
        ...
        <div className="scroll-container flex flex-1 flex-col gap-3">
          {strategies.map((strategy) => (
            <TradeStrategyCard
              ...
              onRestart={() => restartStrategy(strategy.strategy_id)}
