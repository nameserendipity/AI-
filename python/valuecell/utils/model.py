...
from agno.models.deepseek import DeepSeek as AgnoDeepSeekModel
...
def model_should_use_json_mode(model: AgnoModel) -> bool:
    ...
    try:
        ...
        # Dedicated DeepSeek models - structured outputs are not reliably supported
        if (
            AgnoDeepSeekModel
            and provider == AgnoDeepSeekModel.provider
            and name == AgnoDeepSeekModel.name
        ):
            logger.debug("Detected DeepSeek model - using JSON mode")
            return True
