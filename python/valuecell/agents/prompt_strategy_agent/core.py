...
class PromptBasedStrategyAgent(BaseStrategyAgent):
    ...
    async def _build_features_pipeline(
        ...
        if self._is_astock_request(request):
            return AStockAnalysisFeaturesPipeline(request=request)
    ...
    def _is_astock_request(self, request: UserRequest) -> bool:
        symbols = request.trading_config.symbols
        if not symbols:
            return False
        if request.exchange_config.exchange_id:
            exchange_id = request.exchange_config.exchange_id.lower()
            if exchange_id in {"astock", "a-share", "ashare", "cn-stock"}:
                return True
