...
class TradingConfig(BaseModel):
    ...
    dynamic_pool_enabled: bool = Field(
        default=True,
        description="Whether to merge dynamic crypto pool candidates into symbols.",
    )
    dynamic_pool_limit: int = Field(
        default=5,
        description="Maximum dynamic symbols to merge into each strategy runtime.",
        ge=0,
        le=20,
    )
    ...
    def validate_symbols(cls, v):
        ...
        if len(v) > 5:
            raise ValueError("LLM strategy analysis is limited to at most 5 symbols")
