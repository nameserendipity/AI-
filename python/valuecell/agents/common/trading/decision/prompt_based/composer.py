...
from ..astock_guardrails import AStockDecisionGuardrails
...
class LlmComposer(BaseComposer):
    ...
    async def compose(self, context: ComposeContext) -> ComposeResult:
        ...
        guarded = AStockDecisionGuardrails(self._request).apply(context, plan)
        normalized = self._normalize_plan(context, guarded.plan)
        rationale = guarded.plan.rationale
        return ComposeResult(instructions=normalized, rationale=rationale)
    ...
    def _build_llm_prompt(self, context: ComposeContext) -> str:
        ...
        astock_analysis = features.get("astock_analysis", [])
        ...
        payload = prune_none(
            {
                ...
                "summary": summary,
                "market": market,
                "astock_analysis": astock_analysis,
                "features": features,
                "positions": positions,
        ...
        instructions = (
            ...
            "For A-share symbols, astock_analysis is the primary market context: "
            "use analysis.bias, technical.trend, sentiment.bias, support/resistance, "
            "and risk.flags before proposing trades. "
