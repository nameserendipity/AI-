...
class DefaultDecisionCoordinator(DecisionCoordinator):
    ...
    def __init__(
        ...
        *,
        ...
        self._base_symbols = list(dict.fromkeys(base_symbols or request.trading_config.symbols))
    ...
    def _refresh_dynamic_symbols(self) -> None:
        """Refresh the effective symbols from the latest dynamic crypto pool."""
        try:
            from .runtime import _merge_dynamic_crypto_pool

            merged = _merge_dynamic_crypto_pool(
                self._request,
                base_symbols=self._base_symbols,
            )
        except Exception:
            logger.exception(
                "Failed to refresh dynamic crypto pool symbols for strategy_id={}",
                self.strategy_id,
            )
            return
        ...
        self._symbols = list(dict.fromkeys(merged))
        if hasattr(self._features_pipeline, "set_symbols"):
            self._features_pipeline.set_symbols(self._symbols)
