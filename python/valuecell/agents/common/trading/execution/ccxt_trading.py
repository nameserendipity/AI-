...
def _get_ccxt_proxy_url() -> Optional[str]:
    """Return a single proxy URL for CCXT to avoid conflicting settings."""
...
class CCXTExecutionGateway(BaseExecutionGateway):
    ...
    async def _get_exchange(self) -> ccxt.Exchange:
        ...
        try:
            ...
            logger.warning(
                f"鈿狅笍 Could not set position mode ({self.position_mode}) on {self.exchange_id}: {e}"
            )
    ...
    def _build_order_params(self, inst: TradeInstruction, order_type: str) -> Dict:
        ...
        try:
            ...
            if mode in ("oneway", "single", "net"):
                ...
                if removed:
                    logger.debug(
                        f"馃Ч Oneway mode: stripped {removed} from order params"
                    )
        except Exception:
            pass
    ...
    async def _get_free_usdt_okx(self, exchange: ccxt.Exchange) -> Optional[float]:
        ...
        try:
            ...
            logger.warning(f"鈿狅笍 Could not fetch OKX trading balance: {e}")
            return None
        ...
        try:
            ...
            if "fee" in order and order["fee"]:
                ...
                if cost is not None and cost > 0:
                    fee_cost = float(cost)
                    logger.debug(f"  馃挵 Fee from CCXT unified field: {fee_cost}")
                    return fee_cost
            ...
            if self.exchange_id == "binance":
                ...
                if fills:
                    for fill in fills:
                        ...
                        # If fee is in BNB or other asset, log it but don't convert
                            ...
                            logger.info(
                                f"  馃挵 Fee paid in {commission_asset}: {commission}"
                            )
                            # Could implement conversion logic here if needed

                    if fee_cost > 0:
                        logger.debug(f"  馃挵 Fee from Binance fills: {fee_cost}")
                        return fee_cost
                ...
                if fee_str:
                    ...
                    logger.debug(f"  馃挵 Fee from OKX info: {fee_cost}")
                    return fee_cost
                ...
                if cum_fee:
                    ...
                    logger.debug(f"  馃挵 Fee from Bybit info: {fee_cost}")
                    return fee_cost
                ...
                if fee_str:
                    ...
                    logger.debug(f"  馃挵 Fee from Gate.io info: {fee_cost}")
                    return fee_cost
                ...
                if fee_str:
                    fee_cost = float(fee_str)
                    logger.debug(f"  馃挵 Fee from KuCoin info: {fee_cost}")
                    return fee_cost
                ...
                if fills:
                    ...
                    if fee_cost > 0:
                        logger.debug(f"  馃挵 Fee from MEXC fills: {fee_cost}")
                        return fee_cost
                ...
                if fee_detail:
                    ...
                    if total_fee > 0:
                        fee_cost = total_fee
                        logger.debug(f"  馃挵 Fee from Bitget info: {fee_cost}")
                        return fee_cost
                ...
                if fee_str:
                    fee_cost = float(fee_str)
                    logger.debug(f"  馃挵 Fee from Hyperliquid info: {fee_cost}")
                    return fee_cost
            ...
            if fee_cost == 0.0 and filled_qty > 0 and avg_price > 0:
                ...
                logger.debug(
                    f"  馃挵 No fee information found for {symbol} on {self.exchange_id}"
                )

        except Exception as e:
            logger.warning(f"  鈿狅笍 Error extracting fee for {symbol}: {e}")
    ...
    async def execute(
        ...
        """
        if not instructions:
            logger.warning("鈿狅笍 CCXTExecutionGateway: No instructions to execute")
            return []

        logger.info(
            f"馃挵 CCXTExecutionGateway: Executing {len(instructions)} instructions"
        )
        ...
        for inst in instructions:
            ...
            logger.info(
                f"  馃摛 Processing {inst.instrument.symbol} {side.value} qty={inst.quantity}"
            )
            try:
    ...
    def _apply_exchange_specific_precision(
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"  鈿狅笍 Amount {amount} failed precision check for {symbol}: {e}"
                )
                amount = 0.0
            ...
            if self.exchange_id == "hyperliquid":
                ...
                if price is not None and price_precision == 1.0:
                    ...
                    logger.debug(
                        f"  馃敘 Hyperliquid: Rounded price to integer {price} for {symbol}"
                    )

            return amount, price

        except Exception as e:
            logger.warning(f"  鈿狅笍 Precision application failed for {symbol}: {e}")
    ...
    async def _submit_order(
        ...
        try:
            ...
            logger.warning(f"鈿狅笍 Minimum check failed for {symbol}: {e}")
            reject_reason = f"minimum_check_failed:{e}"
        if reject_reason is not None:
            logger.warning(f"  馃毇 Skipping order due to {reject_reason}")
            return TxResult(
                instruction_id=inst.instruction_id,
                instrument=inst.instrument,
                side=local_side,
        ...
        if self.exchange_id == "okx":
            try:
                ...
                logger.warning(
                    f"鈿狅笍 OKX margin precheck failed, proceeding without precheck: {e}"
                )
        ...
        if self.exchange_id == "binance":
            try:
                ...
                if not is_close:
                    ...
                    if is_contract and is_linear:
                        ...
                        if (
                            ...
                            logger.warning(
                                f"  馃毇 Skipping order due to {reject_reason}"
                            )
                            return TxResult(
                                instruction_id=inst.instruction_id,
                                instrument=inst.instrument,
                ...
                logger.warning(
                    f"鈿狅笍 Binance USDT-M margin precheck failed, proceeding without precheck: {e}"
                )
        ...
        try:
            ...
            if mode in ("oneway", "single", "net"):
                ...
                if removed:
                    logger.debug(
                        f"馃Ч Oneway mode (post-override): stripped {removed} from order params"
                    )
        except Exception:
            pass
        ...
        if self.exchange_id == "hyperliquid" and order_type == "market":
            try:
                logger.debug(
                    "  馃搳 Hyperliquid: Converting market order to IoC limit order"
                )
                ...
                if price > 0:
                    ...
                    logger.debug(
                        f"  馃挵 Using IoC limit order: {side} @ {price} (slippage: {slippage_pct:.2%})"
                    )
                else:
                    logger.warning(
                        f"  鈿狅笍 Could not determine market price for {symbol}, will try without price"
                    )
            except Exception as e:
                logger.warning(f"  鈿狅笍 Could not setup Hyperliquid market order: {e}")
                # Fallback: let exchange handle it

        # Create order
        try:
            logger.info(
                f"  馃敤 Creating {order_type} order: {side} {amount} {symbol} @ {price if price else 'market'}"
            )
            logger.debug(f"  馃搵 Order params: {params}")
            ...
            logger.info(
                f"  鉁?Order created: id={order.get('id')}, status={order.get('status')}, filled={order.get('filled')}"
            )
        except Exception as e:
            error_msg = str(e)
            logger.error(f"  鉂?ERROR creating order for {symbol}: {error_msg}")
            logger.error(
                f"  馃搵 Failed order details: side={side}, amount={amount}, price={price}, type={order_type}"
            )
            logger.error(f"  馃搵 Failed order params: {params}")
        ...
        if order_type == "market":
            ...
            if order_id and exchange.has.get("fetchOrder"):
                try:
                    ...
                    logger.info(
                        f"  鈴?Waiting 0.5s for market order {order_id} to fill..."
                    )
                    ...
                    logger.info(
                        f"  馃搱 Order status after fetch: filled={order.get('filled')}, average={order.get('average')}, status={order.get('status')}"
                    )
                except Exception as e:
                    # If fetch fails, use original order response
                    logger.warning(
                        f"  鈿狅笍 Could not fetch order status for {symbol}: {e}"
                    )
        ...
        logger.info(
            f"  馃搳 Final parsed: filled_qty={filled_qty}, avg_price={avg_price}"
        )
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"鈿狅笍 Connection test failed for {self.exchange_id}: {e}")
            return False
    ...
    async def fetch_positions(self, symbols: Optional[List[str]] = None) -> List[Dict]:
        ...
        try:
            ...
            logger.warning(f"鈿狅笍 Could not fetch positions: {e}")
            raise
