...
def _require_sec_email() -> str:
    ...
    if not sec_email:
        raise RuntimeError(
            "SEC filing access requires SEC_EMAIL to be configured (for example: your.name@example.com)."
        )
    return sec_email


def _raise_sec_access_error(exc: Exception) -> None:
    message = str(exc)
    if "Both data sources are unavailable" in message:
        raise RuntimeError(
            "SEC data source temporarily unavailable. EDGAR ticker/reference services could not be reached. Verify network access and retry later."
        ) from exc
    raise RuntimeError(f"SEC filing request failed: {message}") from exc


async def _load_company(cik_or_ticker: str) -> Company:
    _require_sec_email()
    try:
        return await asyncio.to_thread(lambda: Company(cik_or_ticker))
    except Exception as exc:  # noqa: BLE001
        _raise_sec_access_error(exc)
...
async def fetch_periodic_sec_filings(
    ...
    company = await _load_company(cik_or_ticker)
...
async def web_search(query: str) -> str:
    """Search the web for the given query with middleware-first fallback."""
