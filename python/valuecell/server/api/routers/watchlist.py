...
from ...services.astock.watchlist_bridge import (
    get_astock_detail_for_watchlist,
    get_astock_price_for_watchlist,
    is_astock_ticker,
)
...
def create_watchlist_router() -> APIRouter:
    ...
    async def get_asset_detail(
        ...
        try:
            if is_astock_ticker(ticker):
                astock_result = await get_astock_detail_for_watchlist(ticker)
                if astock_result is None:
                    raise HTTPException(
                        status_code=404, detail=f"Asset '{ticker}' not found"
                    )
                asset_data = {k: v for k, v in astock_result.items() if k != "success"}
                asset_detail = AssetDetailData(**asset_data)
                return SuccessResponse.create(
                    data=asset_detail, msg="A-share asset details retrieved successfully"
                )
    ...
    async def get_asset_price(
        ...
        try:
            if is_astock_ticker(ticker):
                astock_result = await get_astock_price_for_watchlist(ticker)
                if astock_result is None:
                    raise HTTPException(
                        status_code=404,
                        detail=f"Price data not available for '{ticker}'",
                    )
                price_data = {k: v for k, v in astock_result.items() if k != "success"}
                asset_price = AssetPriceData(**price_data)
                return SuccessResponse.create(
                    data=asset_price, msg="A-share asset price retrieved successfully"
                )
